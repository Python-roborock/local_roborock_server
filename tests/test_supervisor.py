...
def test_release_supervisor_external_tls_skips_certificate_setup(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(server_module, "_connectivity_check", lambda host, port: None)
    monkeypatch.setattr(
        supervisor.certificate_manager,
        "ensure_certificate",
        lambda: (_ for _ in ()).throw(AssertionError("ensure_certificate should not run in external_tls mode")),
    )
    ...
    health = supervisor.runtime_state.health_snapshot()
    service_map = {service["name"]: service for service in health["services"]}
    assert service_map["https_server"]["detail"] == "external_tls:0.0.0.0:8080"
    assert service_map["mqtt_tls_proxy"]["detail"] == "external_tls:0.0.0.0:18883"
